...
export default function ExpensesDetailedCard({ data, editExpenses, onDelete }) {
    ...
    return (
        <div className="bg-white border border-gray-200 group rounded-2xl p-4 shadow-md hover:shadow-md transition-all cursor-pointer">
            ...
            <div className="text-2xl font-bold text-blue-500 mb-2">
                ${styleAmount(data.amount)}
            </div>

            <div className="flex justify-between">
                <div className="flex flex-col gap-1 text-sm text-gray-600 capitalize">
                    <p><span className="font-medium text-gray-700">Type:</span> {data.type}</p>
                    <p><span className="font-medium text-gray-700">Paid by:</span> {data.whoPaid}</p>
                    <p className="text-gray-500 line-clamp-2">Notes: <span className="italic lowercase">{data.notes ? `${data.notes}` : "-"}</span></p>
                </div>
