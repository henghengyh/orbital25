...
describe("Itinerary layout component", () => {
    test("renders heading, input fields, buttons and empty activity layout for a new itinerary", () => {
    ...
    test("renders heading, input fields, buttons and activity layout with activities for existing itinerary", async () => {
        axiosInstance.get.mockResolvedValueOnce({ data: { itinerary: mockItinerary[2] } });
        axiosInstance.get.mockResolvedValueOnce({ data: { isOwner: true } });
    ...
    test("renders save and leave button for collaborators of the itinerary", async () => {
        axiosInstance.get.mockResolvedValueOnce({ data: { itinerary: mockItinerary[0] } });
        axiosInstance.get.mockResolvedValueOnce({ data: { isOwner: false } });
        mockParams.mockReturnValue({ id: "1" });
    ...
    test("renders invite modal with collaborators after clicking invite collaborators for existing itinerary", async () => {
        ...
        axiosInstance.get.mockResolvedValueOnce({ data: { isOwner: true } });
        axiosInstance.get.mockResolvedValueOnce({ data: { owner: mockUser1 } });
        axiosInstance.get.mockResolvedValueOnce({ data: { collaborators: [mockUser2, mockUser3] } });
    ...
    test("only show first 3 collaborators when itinerary has more than 3 collaborators", async () => {
        ...
        axiosInstance.get.mockResolvedValueOnce({ data: { isOwner: true } });
        axiosInstance.get.mockResolvedValueOnce({ data: { owner: mockUser1 } });
        axiosInstance.get.mockResolvedValueOnce({ data: { collaborators: [mockUser1, mockUser2, mockUser3, mockUser2, mockUser3] } });
        mockParams.mockReturnValue({ id: "2" });
